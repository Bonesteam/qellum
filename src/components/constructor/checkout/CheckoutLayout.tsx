...
export default function CheckoutLayout({ checkout, success, subtotal, vatAmount, total, form }: Props) {
  ...
  const planKey = checkout.planId?.replace("price_", "").toLowerCase();
  const planName = planNames[planKey] || "Custom Plan";
        ...
        <div className={styles.content}>
          {success ? (
            ...
            >
              <h2
                className={styles.h2}
                style={{ color: "#4f46e5", marginBottom: "0.75rem" }}
              >
                Your order has been created successfully!
              </h2>
              <p className={styles.smallTextSuccess}>
                Tokens will be credited automatically once payment is confirmed.
            ...
            <div className={styles.grid}>
              <div>
                ...
                >
                  <p>
                    You are purchasing{" "}
                    <strong>{checkout.description}</strong>. Tokens credited
                    after approval.
                  </p>
                  <p style={{ marginTop: "0.5rem" }}>
                    Invoice sent to <strong>{checkout.email}</strong>.
                  </p>
                </div>
              </div>
